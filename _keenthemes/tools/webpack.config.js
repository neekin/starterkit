...
const assetDistPath = distPath + "/../../lib/assets";
...
function getEntryFiles() {
  const entries = {
    ...
    "css/style.bundle": [
      ...
      "./" + path.relative("./", srcPath) + "/sass/plugins.scss",
    ],
    "js/scripts.bundle": "./webpack/scripts" + (demo ? "." + demo : "") + ".js",
  };

  // Custom 3rd party plugins
  (glob.sync("./webpack/{plugins,js}/custom/**/*.+(js)") || []).forEach(
    (file) => {
  ...
  // Widgets js
  entries["js/widgets.bundle"] = glob
    .sync(entryPattern)
  ...
  if (dev) {
    ...
    (
      glob.sync(path.relative("./", coreSrcPath) + "/js/custom/**/!(_)*.js") ||
      []
    ).forEach((file) => {
      entries[file.replace(/.*js\/(.*?)\.js$/gi, "js/$1")] = "./" + file;
    });
    ...
    entries["js/widgets.bundle"] =
      glob.sync(path.relative("./", coreSrcPath) + "/js/widgets/**/!(_)*.js") ||
      [];
  }
  ...
  return {
    ...
    resolve: {
      alias: {
        ...
        "@form-validation":
          (dev ? coreSrcPath : srcPath) + "/plugins/@form-validation/cjs",
      },
      extensions: [".js", ".scss"],
      fallback: {
        util: false,
